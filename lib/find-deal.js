...
// Bir "scrape" işi bitir bitmez aynı scrapeId ile "interact" çağrıldığında,
// Firecrawl tarafında iş henüz sorgulanabilir hale gelmemiş olabiliyor
// ("Error: Job not found"). Bu geçici durumu birkaç kez, artan bekleme
// süresiyle yeniden deneriz.
async function runFirecrawlWithRetry(args, { retries = 3, delayMs = 2500 } = {}) {
  for (let attempt = 0; ; attempt++) {
    try {
      return runFirecrawl(args);
    } catch (err) {
      if (attempt >= retries || !/Job not found/i.test(err.message)) throw err;
...
async function findDiscountedProducts({ tmpFile }) {
  ...
  const extractOut = await runFirecrawlWithRetry(['interact', extractPrompt, '-s', scrapeId]);
